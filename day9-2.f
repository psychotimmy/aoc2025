...
C
      CALL INSORT(RECTS,MAXRECTS,NRECTS)
C
...
C     Put rectangle coordinates into order L->R and Top -> Bottom
      ...
      ELSE
        ...
        YB=Y1
      ENDIF
      WRITE(*,10)"Rectangle TL is ",XL,YT
      RTEST(1)=CMPLX(XL,YT)
      WRITE(*,10)"Rectangle TR is ",XR,YT
      RTEST(2)=CMPLX(XR,YT)
      WRITE(*,10)"Rectangle BL is ",XL,YB
      RTEST(3)=CMPLX(XL,YB)
      WRITE(*,10)"Rectangle BR is ",XR,YB
      RTEST(4)=CMPLX(XR,YB)
      WRITE(*,*)""
C
      L1=1
...
C       Boundary X constant, Y varies
     ...
     +        ((L2.GT.YT).AND.(L2.LT.YB))) THEN
            CHECKRECT=.FALSE.
            WRITE(*,10)"Vertical boundary encroaches ",VX,L2
            GOTO 999
          ENDIF
        ENDDO
C       Boundary Y constant, X varies
     ...
     +        ((L2.GT.XL).AND.(L2.LT.XR))) THEN
            CHECKRECT=.FALSE.
            WRITE(*,10)"Horizontal boundary encroaches ",L2,HY
            GOTO 999
          ENDIF
        ENDDO
