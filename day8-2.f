...
C
C     IF MAXPTS=1000, these arrays are 499,500 elements each!
      COMPLEX*8 DISTANCES((MAXPTS*(MAXPTS-1))/2)
      COMPLEX*8 REFS((MAXPTS*(MAXPTS-1))/2)
      COMPLEX*8 CLOSEREFS((MAXPTS*(MAXPTS-1))/2)
      INTEGER*8 X(MAXPTS),Y(MAXPTS),Z(MAXPTS),PTS,PAIRS
      INTEGER*8 TOTAL,SX,SY,SZ
      INTEGER*8 CALCGROUP
      INTEGER*8 L1,L2,PAIRNO
C
   10 FORMAT(A)
   30 FORMAT(A,I16)
   40 FORMAT(A,2F14.0)
C
      WRITE(*,10)"Advent of Code 2025 day 8, part 2"
      WRITE(*,10)" "
C
...
C     the real part of the complex number held in DISTANCES
      CALL INSORT(DISTANCES,PAIRS)
C     Get the references of all the pairs we want to examine
      DO L1=1,PAIRS
        IDX=AIMAG(DISTANCES(L1))
        CLOSEREFS(L1)=REFS(IDX)
      ENDDO
C     Group the PAIRS nearest junction boxes into the largest
C     possible circuits, stop at the first time we have MAXPTS junction
C     boxes (sample = 20, puzzle = 1,000) in a single circuit. Can safely
C     start the iteration at MAXPTS for both the sample and puzzle.
      PAIRNO=MAXPTS
  700 CONTINUE
C     WRITE(*,*)"Pair number ",PAIRNO
C     Sort the pairs by their REAL component - makes grouping easier
      CALL INSORT(CLOSEREFS,PAIRNO)
      TOTAL=CALCGROUP(CLOSEREFS,PAIRNO)
      IF (TOTAL.NE.MAXPTS) THEN
        PAIRNO=PAIRNO+1
        GOTO 700
      ENDIF
C     We have the answer - look up the X coordinates of the pair by
C     working through a lookup of DISTANCES, REFS and X.
      WRITE(*,10)" "
      WRITE(*,30)
     +   "Junction box pairs in one circuit is",PAIRNO
      WRITE(*,40)"Last 2 junction boxes have distance,reference of ",
     +   DISTANCES(PAIRNO)
      WRITE(*,40)"Junction box pair references are ",
     +   REFS(INT(AIMAG(DISTANCES(PAIRNO))))
      WRITE(*,30)"First X value is ",
     +   X(INT(REAL(REFS(INT(AIMAG(DISTANCES(PAIRNO)))))))
      WRITE(*,30)"Second X value is ",
     +   X(INT(AIMAG(REFS(INT(AIMAG(DISTANCES(PAIRNO)))))))
      WRITE(*,*)
      WRITE(*,30)"Multiplied together gives the answer of",
     +   X(INT(REAL(REFS(INT(AIMAG(DISTANCES(PAIRNO)))))))*
     +   X(INT(AIMAG(REFS(INT(AIMAG(DISTANCES(PAIRNO)))))))
      END
C
...
C       WRITE(*,*)"Number of groups was ",NOTE1," now",NOTE2
      ...
      ENDIF
C     WRITE(*,*)"Number of groups was ",NOTE1," now",NOTE2
C     Step 5 - find the largest group
      T1=0
      DO L1=1,1000
        ...
        ENDIF
      ENDDO
      WRITE(*,10)"Largest circuit is of size ",T1
C     Only want the largest circuit size returned for part 2
      CALCGROUP=T1
      RETURN
      END
C     
